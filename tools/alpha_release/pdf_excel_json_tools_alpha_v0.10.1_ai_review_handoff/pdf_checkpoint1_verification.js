...
async function main() {
  ...
  check('quantity_sidecar_binding_core.js SHA-256 matches Checkpoint 0 baseline (84144dbd...)',
    bindingCoreHash === '84144dbdc5c6c0cd8e719ce282260d13b1f4624ecdf3ea0ef8ff86117ed2243a', bindingCoreHash);
...
async function runScenarios(browser, tempDir, bindingCoreBuf) {
  ...
  // 拒否系: それぞれ独立に、正常な取込前状態から試す(discard→復元は不要、各変異は
  // 別recordや別値を壊すだけなのでdataはまだ書き換わっていないことを都度確認する)。
  async function attemptRejectedImport(mutateFn, label) {
    const result = await page.evaluate(async (mutateFnSrc) => {
      ...
      const beforeState = JSON.stringify(data._tool_state?.review || {});
      let threw = false, message = '';
      try { await v30ApplyPdfAiReviewObject(answer, 'A'); } catch (e) { threw = true; message = e.message; }
      const afterState = JSON.stringify(data._tool_state?.review || {});
      return { threw, message, stateUnchanged: beforeState === afterState };
    }, mutateFn);
    check(`AI回答取込 拒否系[${label}]: 全件拒否(例外送出)`, result.threw === true, result);
    check(`AI回答取込 拒否系[${label}]: 拒否時の部分更新0件(review state不変)`, result.stateUnchanged === true, result);
  }
