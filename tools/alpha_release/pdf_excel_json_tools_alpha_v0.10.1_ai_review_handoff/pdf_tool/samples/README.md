...
- **`sample_input_table.pdf`**：2列以上に分かれた行の連続を表として検出する能力を確認します。
  ...
  と判定することも確認済みです。
- **`sample_input_unextractable.pdf`**：テキスト層抽出が不十分な場合にOCRへフォールバックし、
  本α版ではOCR（Tesseract.js）を意図的に同梱していないためOCRも失敗し、結果として
  「生成に失敗しました: OCRエンジン（Tesseract.js）の読み込みに失敗しました。インターネット接続を
  確認してください。」という明示的なエラーが表示されることを確認します。この際、変換前の`data`は
  変更されず、α評価パネルが変換成功として偽の件数を表示しないことも確認済みです。
