...
4. 3節の比較レコード組み立てを実装し、6節の回帰テストのうち8〜19番、および4・6番を検証する。**Phase B-1（3.2・3.3節、4 ID保持・厳密結合）・Phase B-2（3.4節段階1、同一次元候補バケット／異次元監査バケット）は完了、6節15・17・19番は検証済み**。段階2以降（3.4節2〜4番）・一対多規則（5番）・逐次絞り込み後の最大候補数打ち切り（6番）・`trace-comparison/1.0-rc1`レコード自体の生成は未着手。
