...
**この改訂までに完了した項目**（上記条件との対応）：条件5（§11の機械検証・deep-equal自動化）は完了。条件6は`content_hash`/`quantity_id`・ハッシュ入力構築契約・3経路同値性について完了、PDF側の`dataset_signature`生成・検証も完了（Excel側・比較エンジン側は未完了、上表）。PDF側の`quantity-annotation/1.0-rc1`生成（2.2節）はフェーズAで実装・実ブラウザ検証済み（`quantity_annotation_pdf_verification.js`24件・`quantity_annotation_ported_lib_check.js`5件）。`quantity-annotation/1.0-rc1`のJSON Schema（`quantity_annotation_schema_v1.json`）も作成し、実生成物を検証済み（6節25番の一部）。条件1〜4・7は引き続き未達（Excel側・比較エンジン側の回帰テストは設計のみで未実装、`evaluateAutoApplicable()`シグネチャ変更は未着手、`trace-comparison`側の本体コード実装は未着手）。
