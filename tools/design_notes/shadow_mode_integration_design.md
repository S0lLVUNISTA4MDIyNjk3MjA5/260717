...
> - `quantity_sidecar_binding_core.js`の`generateDimensionCandidates()`として実装し、`quantity_dimension_candidate_verification.js`（45件、実fixtureでのend-to-end確認を含む）・`quantity_sidecar_binding_browser_verification.js`（実UI経由、8件追加）で回帰確認した。20×20の異次元合成データが1件の圧縮記録（`excluded_pair_count:400`）になること、UIステータス表示の件数が診断APIの実際の値と一致し陳腐化しないこと（表示だけが古いtraceMatrixRowsを参照し続ける退行を意図的に注入し検出できることを確認済み）を含む。
...
12.（**手動追加はPhase B-1完了、付け替えは未確認**）手動追加・付け替え（`traceMatrixRows`が通常照合以外の経路で生成された場合）でも`requirement_trace_id`/`actual_trace_id`が正しく保持されること
...
19. 数量が多いレコード同士の組み合わせで候補数が`N×M`へ単純に膨張しないこと（3.4節6番の打ち切り規則の検証）。**異次元同士の除外については15番の圧縮バケットで組み合わせ爆発を防いだ（Phase B-2完了）が、同一次元同士は依然`N×M`件の候補をそのまま生成する**（段階1を通過した候補への最大候補数の打ち切り自体は3.4節6番のまま未確定・未実装）
...
4. 3節の比較レコード組み立てを実装し、6節の回帰テストのうち8〜19番、および4・6番を検証する。**Phase B-1（3.2・3.3節、4 ID保持・厳密結合）・Phase B-2（3.4節段階1、次元候補生成）は完了、6節15・17番は検証済み**。段階2以降（3.4節2〜4番）・一対多規則（5番）・最大候補数打ち切り（6番）・`trace-comparison/1.0-rc1`レコード自体の生成は未着手。
