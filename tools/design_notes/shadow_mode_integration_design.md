...
**ID・ハッシュ関連**：
1.（**PDF側完了**）1文に複数数量がある場合に、正しい`quantity_id`で`analyses[]`が分かれること（`quantity_annotation_pdf_verification.js`で確認）
2.（**PDF側完了**）同一表記の数量（例：「50 °C」が同じ文に2回出現）が異なる`source_span`で区別されること（`occurrence_index`ではなく`source_span`が識別の根拠になっていることを、実際にPlaywrightで確認済み。`quantity_extraction_prototype.js`側の回帰テストで既に検証、2.0節）
3.（**PDF側完了**）`content_hash`のハッシュ対象（本文・タグ）をそれぞれ単独で変更すると、ハッシュ値が不一致になること（本文だけを変更対象にしていないかの検証）。列名・対象セルはExcel側固有のため未実装（Task #2）
4. JSONオブジェクトのプロパティ順だけを変えても`content_hash`/`dataset_signature`が同じ値になること（ハッシュ対象を正規化してから計算していることの検証。`v12CanonicalJson()`のキーソート仕様に依拠しており、専用テストは未実装）
5.（**PDF側完了**）`_trace_records`のレコード順を変更しても`dataset_signature`が同一値になること（`trace_id`昇順への正規化により順序非依存と仕様化した。`quantity_annotation_pdf_verification.js`で確認）
6. 旧`ruleset_version`のquantity-annotationファイルを現行ルールで読み込むと、警告または再生成要求になること（未実装。読み込み側＝比較エンジンが未実装のため）
7.（**PDF側完了**）`_trace_records`の再生成後（同一入力での再実行）も`quantity_id`が変化しないこと（2.0節の内容ベースID規則の安定性そのものの検証。`quantity_annotation_pdf_verification.js`で確認。`comparison_id`は比較エンジン未実装のため未検証）
...
- **本体3ツールへの実装は一部着手**：PDF側の数量注釈シャドー出力（2.2節）はフェーズAで実装・実ブラウザ検証済み（`spec_to_json_conversion_tool_v1.18.html`の`buildQuantityAnnotationSidecar()`/`v12ExportQuantityAnnotationSide()`、新設ボタン`#btn-quantity-annotation-export`/`-b`）。`json_ab_trace_matching_tool_v12.1.15.html`側の`requirement_trace_id`/`actual_trace_id`/`matcher_a_id`/`matcher_b_id`保持（3.2節）、Excel側の数量注釈シャドー出力（2.3節）、比較レコード組み立て（3節）はいずれも未着手のまま。
...
| `dataset_signature`の生成・検証（Excel側・比較エンジン側） | 未実装 |
| 本体3ツール（ブラウザ）でのハッシュ生成（**PDF側**） | **実装・実ブラウザ検証済み**（`v12ComputeRecordContentHash()`/`v12QuantityIdDigest()`/`v12ComputeDatasetSignature()`が、既存の`v12Sha256()`/`v12HashParts()`/`v12CanonicalJson()`/`v12Id()`をそのまま再利用。新規のハッシュユーティリティは追加していない） |
| 本体3ツール（ブラウザ）でのハッシュ生成（Excel側・比較エンジン側） | 未実装 |
...
**この改訂までに完了した項目**（上記条件との対応）：条件5（§11の機械検証・deep-equal自動化）は完了。条件6は`content_hash`/`quantity_id`・ハッシュ入力構築契約・3経路同値性について完了、PDF側の`dataset_signature`生成・検証も完了（Excel側・比較エンジン側は未完了、上表）。PDF側の`quantity-annotation/1.0-rc1`生成（2.2節）はフェーズAで実装・実ブラウザ検証済み（`quantity_annotation_pdf_verification.js`35件・`quantity_annotation_ported_lib_check.js`5件。同一スナップショット保証は`v12BuildTrace()`呼び出し回数のモンキーパッチ計測で直接検証し、意図的な退行注入で実際に検出できることも確認した）。`quantity-annotation/1.0-rc1`のJSON Schema（`quantity_annotation_schema_v1.json`）も作成し、`quantity`/`evidence`の内部構造(区間・単位・抽出信頼度の必須フィールド)まで検証する形に強化した上で実生成物を検証済み（6節25番の一部）。トップレベルに`id_hash_algorithm: "SHA-256/128"`フィールドを追加し、文書・実出力・Schemaの3者を一致させた。条件1〜4・7は引き続き未達（Excel側・比較エンジン側の回帰テストは設計のみで未実装、`evaluateAutoApplicable()`シグネチャ変更は未着手、`trace-comparison`側の本体コード実装は未着手）。
