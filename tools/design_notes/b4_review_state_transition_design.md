...
この派生値はoverlayへ保存しない。`automatic_judgement`の`state`、`satisfied`、`judgement_source`、`human_confirmed`を変更しない。`deriveAllReviewed(session, comparisonId)`は5対象すべてが`reviewed`なら`true`を返し、保存フィールドにはしない。
