...
> - 本改訂→`trace-comparison/1.0-rc2`（Phase B-3a/B-3b実装、`c393691`承認後、2026-07-22）：本節が設計した`-rc1`（`automation`/`comparison`(null許容)/`lowGap`・`highGap`/単一`review.confirmed`系）は、B-2.5/B-2.6で確定した契約(`numeric_comparison`常時保持・`auto_applicability`・`automatic_judgement`・`needs_confirmation`)と非互換なため、`schema_version`を`trace-comparison/1.0-rc2`へ改めた（本節・以降の`-rc1`本文は歴史的記録として保持し、上書きしない）。`quantity_sidecar_binding_core.js`の`generateTraceComparisonRecordSet()`が正式実装であり、フィールド対応・設計判断の詳細は`shadow_mode_integration_design.md`の該当訂正段落を参照。
...
> - `7b4fc7b`レビュー（2026-07-21）：round1で導入した全体上限(`totalCandidateLimit`)自体に、なお重大3件・中1件が残っていた。(1) 全体上限の判定を「全バケット・全conceptの候補生成が完了した後」に行っており、判定材料自体は減ったが「生成してから破棄する」構造は残っていた、(2) 判定材料が「切り詰め後の実現候補数」であり、多数の小さなグループ（例: 100グループ×潜在100件を`candidateLimit=1`で切り詰め）では真の潜在合計（10,000件）を過小評価し超過を検知できない、(3) per-group切り詰め発生時に`ready:true`のまま不完全な候補集合を返しており、後段（未実装）がこれを完全な結果として扱いうる。修正: (1)(2)は候補生成をPass 1（潜在ペア数の合計のみ、候補オブジェクトは生成しない）とPass 2（実際の候補生成）に分離し、潜在合計が`totalCandidateLimit`を超える場合はPass 2を実行せずfail closedする形へ変更、`totalCandidateLimit`専用の上限(`MAX_SAFE_TOTAL_CANDIDATE_LIMIT=10,000,000`)を新設し既定値も2,000へ引き上げた。(3)はレビューが認めた代替案（`result_complete:false`）を採用し、per-group単位の完全fail closedは実文書での実用性を損なうという理由で見送った（1件の異常なレコードのために比較実行全体を止めない）。段階3以降の実装時は`result_complete===false`の結果を確定結果として扱わない契約とする。詳細・判断理由は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `95af0db`レビュー（2026-07-21）：round2で導入した「潜在合計で判定するPass 1」自体に、なお重大3件・中1件が残っていた。(1) 「潜在ペア数(探索空間)」と「実体化候補数(実際にメモリへ載る量)」を混同しており、`totalCandidateLimit`の検証上限を10,000,000まで許容していたため、`candidateLimit=10,000`・1,000グループのような設定でPass 2が実際に1,000万件を実体化できてしまう、(2) 上限超過確定後もバケット走査を最後まで続けており、無駄な記述子蓄積・監査記録生成が残っていた、(3) 全体停止時に「実際には1件も生成していない」のに`candidate_limit_exceeded`(切り詰めた)という事実と異なる監査記録を個々のグループへ出していた。修正: (1)上限を`totalCandidateLimit`(実体化見込み件数、検証上限を100,000へ戻す)と新設`totalPotentialPairLimit`(潜在ペア数合計、検証上限1,000,000,000)に分離、(2)バケット走査ループ自体を1グループ処理するごとに判定し超過時点でlabeled breakにより即座に打ち切る(タイミング非依存の決定的な証明テストを追加)、(3)全体停止経路専用の`candidate_limit_would_exceed`(`materialized_pair_count:0`明示)へ変更し実際に切り詰めた場合の`candidate_limit_exceeded`と区別。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `0957659`レビュー（2026-07-21）：round3修正になお重大2件・中3件が残っていた。(1) `emitConceptGroupCandidates()`が候補を`candidateLimit`件に打ち切る前に`reqIds`/`actIds`全体をconfidence降順へ複製・全件ソートしており、`totalCandidateLimit`(実体化見込み件数の上限)がこの複製・ソートのコスト自体を制限できていなかった(要求側50万件・`candidateLimit=50`でも50万要素をソートしてしまう)、(2) 早期打ち切りの結果がrelations引数の入力順に依存しており、同じrelations集合でも配列順を変えるだけで打ち切り時点の観測値・巻き込まれたグループが変わっていた、(3) materialized・potential両方の上限を同時に超えても片方しか記録しない、(4) 500msのタイミング判定がまだブロッキングだった、(5) 実体化上限の検証上限(100,000)がブラウザでの安全性未検証のまま設定されていた。修正: (1)ソート自体を撤廃(stage 1が既にquantity_id昇順でソート済みのため不要だった)、(2)Pass 1へ入る前にバケットを安定キーで並べ替える、(3)`limit_kinds`(複数形)へ両方記録、(4)タイミング計測を完全に非ブロッキングのログ出力のみへ変更、(5)検証上限を10,000へ引き下げ。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `a9c24bc`承認後（Phase B-2.3a実装、2026-07-21）：3.4節 段階3を「条件候補の整合・comparisonMode導出」を一括りにした未実装ブロックとして記載していたが、実装をさらに2段階へ分割した。段階3-1(`generateConditionResolutions({binding})`)は、数量ごとにPhase A抽出時点で既に計算済みの`analysis.interval_semantics_candidates`を、7節の`resolvePropertyStatus()`と同型の閾値判定(ただし`propertyConfidence`ではなく`modeConfidence`を使う)で`resolved`/`ambiguous`/`unavailable`へ正規化する(候補自体の再生成は行わない。区間意味候補は段階2aのconcept候補とは異なり既にPhase Aで確定済みのため)。段階3-2(`generateConditionAnnotatedComparisonCandidates({binding, relations, ...})`)は、段階2(`generateComparisonCandidates()`)の`comparison_candidates`へ両側の条件解決結果をフラットな4フィールドとして付加する。comparisonResult/conditionResultはbindingから内部で1回ずつ計算し呼び出し側からは受け取らない(B-2.2a round1以来の設計方針を踏襲)。B-2.2b承認時の必須要件どおり、`comparisonResult.ready !== true`または`result_complete !== true`の場合は候補を1件も生成せずfail closedする契約をこの最初の段階3関数で実装し、段階3の最初の回帰テストとして固定した。comparisonMode導出・単位変換・数値比較・区間比較・充足判定は引き続き未実装のまま。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `f77dfca`レビュー（2026-07-21）：段階3-1初回実装に重大2件・中2件、回帰テスト不備1件が見つかった。(1) `value`がSchema上任意の非空文字列であるにもかかわらずconfidence/marginだけでresolvedを判定しており、ruleset v2.19が生成し得ない未知語や`'unknown'`自体が高confidenceで格納されるとresolvedへ昇格し得た、(2) `interval_semantics_candidates`の配列サイズにSchema上の上限がなく、件数検査より前に全件複製・ソートしていた、(3) 付加フィールドがstatus/valueの2個だけで、後段の安全判定(将来のcomparisonMode自動適用)に必要なmargin・否定根拠の有無が失われていた、(4) confidence同点候補の出力順が入力配列の元の順序に依存していた、(5) 実fixtureのend-to-endテストが`ready===true||ready===false`という空虚な検証だった。修正: (1)ruleset v2.19の既存語彙9種のallowlist(`KNOWN_CONDITION_SEMANTICS_VALUES`)を新設し未知語・`'unknown'`をresolved対象から除外、(2)`MAX_INTERVAL_SEMANTICS_CANDIDATES_PER_QUANTITY`(64)を複製・ソート前に検査しfail closed、同じ検査でvalue重複も検出、(3)`hasOpposingEvidence()`を`marginOf()`と並び一字一句移植し`top_confidence`/`margin`/`has_opposing_evidence`を明示フィールド化(段階3-2の付加フィールドも6個へ拡張)、(4)`sortedByConfidenceDesc()`へvalue昇順のtie-breakを追加、(5)実在するrelationを使った意味のあるend-to-endテストへ書き直し。回帰テストを51件から81件へ拡張した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `fdb337a`承認後（Phase B-2.3b実装、2026-07-21）：3.4節 段階3の残り(comparisonMode導出)を段階3-3として実装した。`generateComparisonModeCandidates({binding, relations, ...})`は段階3-2(`generateConditionAnnotatedComparisonCandidates()`)をbindingから内部で計算し、`ready!==true`または`result_complete!==true`ならfail closedする(段階3-2が段階2に課した契約と同じ連鎖)。両側とも条件`resolved`かつ否定根拠(`has_opposing_evidence`)が無い比較候補についてのみ、固定表`COMPARISON_MODE_DERIVATION_TABLE`(semantic_mapping_prototype.js 368-374行目から一字一句移植、乖離検出は`quantity_annotation_ported_lib_check.js`の`checkPortedComparisonModeTable()`)から`(requirement_condition_value, actual_condition_value)`の完全一致だけでmodeを導出する。未解決(`condition_unresolved`)・否定根拠あり(`condition_opposing_evidence`)・対応表に無い組み合わせ(`comparison_mode_unavailable`、`required_capability_domain × achieved_point`を含む)はいずれも推測せずnot_analyzedへ送る。`comparison_mode_confidence`は両側`top_confidence`の`Math.min()`(移植元`deriveComparisonModeCandidate()`と同じ保守的な採用方針)。単位変換・数値比較・区間包含判定・auto applicability・充足判定は引き続き未実装のまま。新規テストファイル(`quantity_comparison_mode_candidate_verification.js`、37件)を追加し、5件の防御をそれぞれ個別に無効化して失敗を確認した上で復元した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `b17d8e0`レビュー（2026-07-21）：初回実装に重大1件・中1件が見つかった。(1) 公開exportした`COMPARISON_MODE_DERIVATION_TABLE`(配列・各entryとも)が凍結されておらず、呼び出し側が`push()`・entry直接書き換えで実行時に変更でき、v2.10で安全側の理由により除外した`required_capability_domain × achieved_point`を復活させられることを実際に確認した(ファイル末尾の`Object.freeze({...})`はAPIオブジェクト自身だけを凍結する浅い凍結であり、内部の配列・オブジェクトまでは凍結しないため)、(2) `condition_unresolved`/`comparison_mode_unavailable`のnot_analyzed監査記録に両側の`top_confidence`/`margin`が引き継がれておらず、除外理由(confidence不足かmargin不足か)をB-2.3bの出力単体から判別できなかった。修正: (1)移植ブロック自体は改変せず、ポート直後に`COMPARISON_MODE_DERIVATION_TABLE.forEach(Object.freeze); Object.freeze(COMPARISON_MODE_DERIVATION_TABLE);`を追加、(2)`auditBase`へ両側の`top_confidence`/`margin`(4スカラー)を追加。回帰テストを37件から46件へ拡張し、表の凍結・監査フィールド配線のそれぞれを個別に無効化すると対応するテストが失敗すること(表の凍結無効化では実際に除外済みペアの候補が生成されることも)を確認した上で復元した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `31a4002`承認後（Phase B-2.4a実装、2026-07-21）：3.4節 段階4の最初の部分として、単位互換性の判定と変換計画の生成だけを行う`generateUnitConversionPlans({binding, relations, ...})`を実装した。段階3-3(`generateComparisonModeCandidates()`)をbindingから内部で計算し、`ready!==true`または`result_complete!==true`ならfail closedする。各候補のquantity_idからbinding内のanalysisを再引きし、片側でも見つからなければ呼び出し全体をfail closedする(`unit_plan_quantity_missing`)。数量値・区間境界へは一切変換を適用しない。単位互換性の分類は独立した純粋関数`classifyUnitConversion(requirementUnit, actualUnit)`に切り出し、canonical一致は`identity`計画、dimension一致・固定の線形変換規則(現時点ではpressureのPa/kPa/MPaのみ、`LINEAR_UNIT_SCALE_TO_BASE`)がある場合は`linear_scale`計画(常にactual→requirement方向)、dimension不一致は`unit_dimension_inconsistent`として呼び出し全体をfail closed(構造的異常、通常到達不能)、未対応canonical・metadata不備はそれぞれ`unit_conversion_unsupported`/`unit_metadata_unsupported`としてnot_analyzedへ送る(推測しない)。`LINEAR_UNIT_SCALE_TO_BASE`はB-2.3bと同じ理由(呼び出し側からの実行時書き換え防止)で内側・外側とも凍結した。B-2.2aのCONCEPT_DICTIONARYにpressure次元の概念が無く、pressure次元の数量はconcept解決でresolvedへ至れないため、comparison_mode_candidateまで到達できない構造的制約があり、Pa/kPa/MPa間の変換計算は`classifyUnitConversion()`を直接呼ぶテストで検証し、`generateUnitConversionPlans()`自体の配線は到達可能なpower/kW経由のend-to-endテストで別途検証する2段構えのテスト設計にした。既存の`coverageGap()`はcanonical不一致で比較不能にした上で数値比較・充足判定まで進む設計であり、単位互換性判定だけを切り出したこの段階とは責務が異なるため呼び出していない。新規テストファイル(`quantity_unit_conversion_plan_verification.js`、55件)を追加し、5件の防御(上流ゲート・表の凍結・metadata不備検査・dimension不一致検査・factor計算の方向)をそれぞれ個別に無効化して失敗を確認した上で復元した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `34c7e9a`レビュー（2026-07-21）：初回実装に重大1件・中3件が見つかった。(1) 既知単位のallowlistが無く、canonical文字列が同一というだけでidentity計画にしており、未登録canonical同士(`psi`×`psi`)や既知canonicalの誤dimension組み合わせ(`kW`をdimension:`voltage`として記録)も正しい単位として誤認することを実際に再現して確認した、(2) `classifyUnitConversion()`がbindingを経由しない純粋関数のまま公開APIとしてexportされており、「公開APIはbinding経由でのみ計算する」信頼境界の外側から呼べる入口になっていた、(3) `unit_plan_quantity_missing`/`unit_dimension_inconsistent`をfail closedへ変換する配線に恒久的な回帰テストが無かった、(4) 単位未対応で除外した`not_analyzed`監査記録からcomparison mode・condition解決の情報が失われていた。修正: (1)`UNIT_DEFS`が実際に定義する(dimension, canonical)の組だけのallowlist`KNOWN_CANONICAL_UNITS_BY_DIMENSION`を新設しidentity判定より前に検証、`UNIT_DEFS`本体との双方向乖離検出も追加、(2)単位規則(`classifyUnitConversion()`・両テーブル)を独立ライブラリ`unit_conversion_rules_prototype.js`へ切り出し、`quantity_sidecar_binding_core.js`は一字一句移植した非公開実装として使い公開APIとしては再exportしない、(3)`unit_plan_quantity_missing`/`unit_dimension_inconsistent`は`analysisByQuantityId()`と上流チェーンが同一のbindingデータを異なる添字方法で読むだけという構造上いかなる binding構築によっても到達不能であることを確認した上で、B-2.3a・B-2.3bの前例どおり一時的なバグ注入による検証を維持(分岐自体は削除せず、防御を保持する判断)、(4)監査記録を`{...candidate}`ベースへ変更しcomparison mode/condition情報を保持。回帰テストを55件から84件へ拡張した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `db770de`レビュー（2026-07-21）：round1修正でも、`isKnownUnit()`と線形変換表のown property検証に重大1件（2巡目）が残っていた。`KNOWN_CANONICAL_UNITS_BY_DIMENSION[key]`の真偽値判定・`key in LINEAR_UNIT_SCALE_TO_BASE[dimension]`はいずれも通常のオブジェクトリテラルが継承する`Object.prototype`のプロパティ(`toString`・`constructor`・`__proto__`・`hasOwnProperty`等)にもtrueを返すため(`Object.freeze()`はこの継承経路を塞がない)、これらをcanonical/dimensionとして渡すと`toString`×`toString`(power)が誤ってidentity計画になり、`toString`×`constructor`(pressure)が関数オブジェクト同士の除算でfactor:NaNのlinear_scale計画を生成することを実際に再現して確認した。修正: `Object.prototype.hasOwnProperty.call()`によるown property専用の`hasOwn()`ヘルパーを新設し、`isKnownUnit()`・線形変換表双方の判定を置き換え、最後の防御として`factor`計算後に`Number.isFinite(factor)`を確認する新設の理由コード`unit_conversion_invalid_factor`も追加した(線形変換表側の`hasOwn()`化と`Number.isFinite`ガードは、`isKnownUnit()`がcanonicalを既に有限個のallowlist済み文字列へ制限した後にしか到達しないため、B-2.4a round1の前例と同じく構造的に到達不能な防御であることをバグ注入で確認した上で保持した)。中1件(`unit_plan_quantity_missing`/`unit_dimension_inconsistent`の一時的バグ注入のみによる検証)はround1の到達不能性の証明を踏まえ対応不要と受理され、もう1件(データテーブルのexport)は設計文書側は当初から正確だったため対話上の説明の明確化のみで、コード・設計文書とも変更なし。回帰テストを84件から91件へ拡張し、継承キー4種の直接テスト・pressureでの異なる継承キー同士のテストに加え、`canonical:'toString', dimension:'power'`を含む不正sidecarが公開パイプライン`generateUnitConversionPlans()`経由でもidentity計画を生成しないend-to-endテストを新設した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `1ad75f9`承認後（Phase B-2.4b実装、2026-07-21）：3.4節 段階4の後半として、`generateUnitConversionPlans()`の各計画を実仕様側の数量値の複製へ適用し、要求側の単位で表した正規化ビューを生成するだけの`generateNormalizedQuantityViews({binding, relations, ...})`を実装した。段階4前半をbindingから内部で計算し、`ready!==true`または`result_complete!==true`ならfail closedする。数量値への計画適用は独立した純粋関数`applyLinearConversion(quantityValue, plan)`に切り出し(`unit_conversion_rules_prototype.js`へ追加、`classifyUnitConversion()`と同じ理由で公開APIとしては再exportしない)、`value*factor+offset`で`kind:'interval'`の`lower`/`upper`(nullは変換せずnullのまま)、`kind:'alternatives'`の`options`各要素を変換する。identity計画でも常に新しいオブジェクトを返す(元の数量値は一切変更しない)。出力の各要素は元のcomparison mode候補・単位変換計画の全フィールドを維持した上で`requirement_quantity_value`・`actual_quantity_value_original`・`actual_quantity_value_normalized`の3フィールドを追加する。数値比較・区間包含判定・gap計算・auto applicability・充足判定は引き続き未実装のまま。B-2.4aと同じ2段構えのテスト設計(pressureの数値計算は`applyLinearConversion()`を直接requireして検証、配線は到達可能なpower/kW経由のend-to-endテストで検証)を踏襲し、新規テストファイル(`quantity_normalized_quantity_view_verification.js`、35件)を追加、4件の防御(上流fail closedゲート・複製契約・`normalized_view_quantity_missing`・`quantity_value_kind_unsupported`への振り分け)をそれぞれ個別に無効化して失敗を確認した上で復元した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `1fbfe90`レビュー（2026-07-21）：初回実装に重大3件・中1件が見つかった。(1) `applyLinearConversion()`が数量値の型・有限性を確認せず`value*factor+offset`を適用しており、JSON Schemaが型検証しない`alternatives.options`(items未定義)にnull/文字列/object/NaN/Infinityが混入していても暗黙変換または非有限値のまま「正規化済み」として出力されることを実際に確認した、(2) requirement側の数量値を一切検証せずそのまま出力しており、後続の数値比較の入力として不適切な値が混入しうる、(3) `alternatives.options`にJSON Schema上サイズ上限が無く、件数検査より前に`.map()`で全件複製していた(B-2.2b/B-2.3aと同種の組み合わせ爆発)、(4) `applyLinearConversion()`は独立exportされる純粋関数だが`plan.factor`/`plan.offset`の有限性・正数性を検証していなかった。修正: (1)`isFiniteNumber()`による変換前後の検証を追加(理由コード`quantity_value_invalid`/`quantity_conversion_non_finite`)、(2)`validateQuantityValueStructure()`をrequirement側にも適用し`side`フィールドで異常側を明示、(3)`MAX_ALTERNATIVE_VALUES_PER_QUANTITY`(64)を全件走査より前に検査(理由コード`quantity_value_limit_exceeded`、Proxyで全件走査への未到達を直接証明)、(4)`plan.factor`/`plan.offset`の有限性・`factor>0`を検証(理由コード`quantity_conversion_plan_invalid`)。`applyLinearConversion()`の戻り値を`null`／生オブジェクトから`{outcome:'converted', value}`／`{outcome:'unsupported', reason_code}`の判別可能な共用体へ変更した。直接テストを12件から27件へ、`quantity_normalized_quantity_view_verification.js`を35件から59件へ拡張し、レビュー必須の18項目(malformed alternatives・オーバーフロー・plan異常・件数上限・両側end-to-end拒否・正常alternativesのend-to-end・監査情報の引き継ぎ等)をすべて含めた。4件の防御をそれぞれ個別に無効化して失敗を確認した上で復元した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `c00cc3a`レビュー（2026-07-21）：round1修正でも重大2件・中1件が残っていた。(1) 0要素の`alternatives`が件数上限検査(`length > MAX`のみ)・空配列への`.every()`(vacuous true)の両方をすり抜け、正常な変換対象として扱われていた(後続の数値比較に使える値が1つも無い)、(2) lower/upper両方がnullの`interval`も「存在する場合だけ検証する」形の検査をすり抜け正常扱いされていた(片側nullは正当だが両側nullは値が無いのと同義)、(3) `validateQuantityValueStructure()`/`applyLinearConversion()`が独立exportされる純粋関数でありながら、null/非オブジェクトの入力に対して例外を投げていた(`quantityValue.kind`・`plan.factor`への無条件アクセス)。修正: (1)(2)件数検査と同じ「要素走査より前」の位置に0要素チェック・両側nullチェックを追加し、新設の理由コード`quantity_value_empty`で拒否(片側nullの正常ケースは回帰しないことも確認)、(3)`validateQuantityValueStructure()`冒頭でquantityValue自体の型を確認し(`quantity_value_invalid`)、`applyLinearConversion()`冒頭でplan自体の型を確認する(`quantity_conversion_plan_invalid`)ようにし、alternatives.optionsが配列であること・interval のlower/upperが非null時にオブジェクトであることも同じ理由コードで確認するようにした。直接テストを27件から36件へ、`quantity_normalized_quantity_view_verification.js`を59件から76件へ拡張し、空alternatives・両側null intervalそれぞれをrequirement/actual両側かつend-to-endで拒否するテストを追加した。3件の防御をそれぞれ個別に無効化して失敗(またはnull入力での実際の例外送出)を確認した上で復元した。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
> - `be47b9d`レビュー（2026-07-21）：`[10,5]`・`[5,5)`等の数学的に空な区間が既存`isEmptyInterval()`と異なる基準で通っていた、境界`inclusive`の型未検証、`selection_semantics`未検証、`options.every()`/`.map()`直接呼び出しがメソッド上書き時に例外化(重大1件・中3件)。修正: `isEmptyInterval()`と同じ判定(`quantity_value_empty`共用)・`isValidBound()`・`selection_semantics`非空文字列検証・添字ループ化。直接テスト36→48件、`quantity_normalized_quantity_view_verification.js` 76→80件。4件の防御を個別無効化して失敗を確認の上で復元。詳細は`shadow_mode_integration_design.md` 3.4節の訂正を参照。
