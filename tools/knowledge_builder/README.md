...
## α0.1.3 是正Checkpoint: Graphフィルタ・集約表示の不具合修正

限定人手評価への移行前の再検証で見つかった2件の不具合を修正しました。新機能の追加は行って
いません。修正はいずれも表示計算のみで、Knowledge Node/Edge・Candidate生成ロジック・
...
- **章・節単位の集約Graphで、Nodeラベル・件数ラベル・マーカーが重なる不具合を修正**: 単純な
  表示領域の拡大・縮小ではなく、配置計算そのものを見直した。Node本体が並ぶ列とEdge描画領域を
  分離し(ラベルは常にNodeの外側、Edgeは中央のコーパス領域に接続)、Edgeの接続点をNode中心では
  なくNode形状の境界に変更し、Nodeラベル・集約Edgeの件数ラベルには不透明な背景を敷いて重なり
  による視認性低下を防いだ。自然な中点が近接/一致する複数の集約線には、決定的なレーン
  オフセット(安定した並べ替えキーに基づく対称オフセット)を割り当て、マーカー同士・件数ラベル
  同士が重ならないようにした。中規模サンプル(章・節単位)でのマーカー間衝突・ラベル間衝突・
  集約Edgeの線とNodeラベルの矩形との交差を実ブラウザの座標情報から検証し、いずれも0件で
  あることを確認している
