...
// HUNK_30 (new, Checkpoint 2-F): the .detail-expand-toggle CSS class (Human UI defect fix -
...
// L3-2 Checkpoint 2-D (UX/DOC-MAJOR-01/UX-MINOR-01 remediation): a Human reviewer using ONLY the
// normal Human UI (never DevTools/console/window.__quantityBindingDiagnostics/Node/source
// inspection) must be able to confirm every field the Human Ground Truth requires for a given
// property resolution: side, property context source, fallback reason, status, resolved concept,
// top candidate confidence. HUNK_39 and HUNK_40 below are RE-TOUCHED IN PLACE (this round's new
// content lands contiguously with, respectively, the Checkpoint 2-C currentQuantityPropertyState()/
// quantityPropertyContextSummary()/PROPERTY_CONTEXT_REASON_LABELS_JA block and the Checkpoint 2-C
// aggregate-summary-line append inside renderQuantityBindingStatus(), so `git diff` against the
// shared pre-head merges them into one hunk each - same re-touch convention as HUNK_5/16/17/19
// above). Three genuinely NEW, non-adjacent hunks are added:
//   HUNK_42 (new): the new #quantityPropertyDetail <div>, inserted immediately after the existing
//            #quantityBindingStatus element (additive sibling, not a new panel/dashboard).
//   HUNK_43 (new): renderQuantityBindingStatus()'s "unmergeable diagnostics" early-return branch
//            also clears the new per-record detail via renderQuantityPropertyDetail(null).
//   HUNK_44 (new): invalidateQuantityBindingSelection() also clears the new per-record detail via
//            renderQuantityPropertyDetail(null), so a stale per-record detail is never left visible
//            after the input selection changes.
// Hunk count is now 41 (38 + 3 new [HUNK_42-44]; HUNK_39/HUNK_40 updated in place, all other hunks
// unchanged). No matching algorithm, Quantity core module, CONCEPT_DICTIONARY, weight, threshold,
// margin, dimension/unit conversion, condition, or numeric-comparison logic is touched by this
// round - purely additive Human-facing presentation reading the SAME already-computed
// generatePropertyResolutions() result (property_context_source/property_context_reason/status/
// concept_id/candidates[0].confidence are read and labeled for display only, never recomputed or
// reweighted). See private_dictionary_p2a4_authorized_matching_diff_guard_selftest_l32_checkpoint2d.js
// for this round's real-diff acceptance + adversarial tamper-rejection tests.

// HUNK_39 (RE-TOUCHED, Checkpoint 2-D): the Checkpoint 2-C currentQuantityPropertyState()/
// quantityPropertyContextSummary()/PROPERTY_CONTEXT_REASON_LABELS_JA block (unchanged, plus the new
// canonical_unique_property label entry) now also covers, in the same contiguous region immediately
// before renderQuantityBindingStatus(): the new PROPERTY_STATUS_LABELS_JA/PROPERTY_SIDE_LABELS_JA/
// PROPERTY_SOURCE_LABELS_JA label maps, the new renderQuantityPropertyDetail() function, and the
// first early-return branch's new renderQuantityPropertyDetail(null) call.
const HUNK_39 = "     };\n   }\n \n+  // L3-2 Checkpoint 2-C: property候補の生成にnearbyTextForRecord()以外(traceMatrixRows等)は\n+  // 不要なため、currentQuantityDimensionState()と異なりtraceMatrixRows確定を待つ必要はない。\n+  // quantityBindingStateがready:trueになった時点でいつでも再計算できる(キャッシュしない -\n+  // currentQuantityDimensionState()と同じ理由、stale化した中間状態の使い回しを避ける)。\n+  function currentQuantityPropertyState() {\n+    if (!quantityBindingState || !quantityBindingState.ready || !globalThis.QuantitySidecarBinding) return null;\n+    return globalThis.QuantitySidecarBinding.generatePropertyResolutions({ binding:quantityBindingState });\n+  }\n+\n+  // property_context_source/property_context_reason(生の機械可読値、checkpoint task §17)を\n+  // 件数集計のみ行う。個々のresolutionそのものは変更しない - Human向け表示は\n+  // renderQuantityBindingStatus()側でこの集計結果からラベル文字列を組み立てるだけで、\n+  // 生の値をローカライズ文字列で置き換えることはしない。\n+  function quantityPropertyContextSummary(state = currentQuantityPropertyState()) {\n+    if (!state || !state.ready) return { phase:'not_generated', ready:false, canonicalCount:0, legacyCount:0, reasonCounts:{} };\n+    const resolutions = state.resolutions || [];\n+    const reasonCounts = {};\n+    let canonicalCount = 0, legacyCount = 0;\n+    resolutions.forEach(r => {\n+      if (r.property_context_source === 'canonical_property') canonicalCount++;\n+      else legacyCount++;\n+      if (r.property_context_reason) reasonCounts[r.property_context_reason] = (reasonCounts[r.property_context_reason] || 0) + 1;\n+    });\n+    return { phase:'property_stage', ready:true, canonicalCount, legacyCount, reasonCounts };\n+  }\n+\n+  // Human向けラベル(表示専用、checkpoint task §9/§17)。property_context_source/\n+  // property_context_reasonという生の機械可読値そのものは一切変更・置換しない。\n+  const PROPERTY_CONTEXT_REASON_LABELS_JA = Object.freeze({\n+    canonical_unique_property:'検証済み一意property',\n+    canonical_property_not_classified:'canonical propertyが未検出',\n+    canonical_property_ambiguous:'canonical property候補が曖昧',\n+    canonical_property_blank:'canonical propertyが空欄',\n+    canonical_bridge_unavailable:'canonical bridgeを利用できない',\n+  });\n+  // L3-2 Checkpoint 2-D: 表示専用ラベル(生のstatus値は一切変更・置換しない、checkpoint task §4)。\n+  const PROPERTY_STATUS_LABELS_JA = Object.freeze({\n+    resolved:'解決済み (resolved)',\n+    ambiguous:'曖昧 (ambiguous)',\n+    unavailable:'利用不可 (unavailable)',\n+  });\n+  const PROPERTY_SIDE_LABELS_JA = Object.freeze({ requirement:'要求側 (requirement)', actual:'実仕様側 (actual)' });\n+  const PROPERTY_SOURCE_LABELS_JA = Object.freeze({ canonical_property:'Canonical propertyを使用', legacy_nearby_text:'従来の周辺文脈を使用' });\n+\n+  // L3-2 Checkpoint 2-D (UX/DOC-MAJOR-01 remediation): a Human reviewer must be able to confirm\n+  // every field the Human Ground Truth requires (side / property context source / fallback reason\n+  // / status / resolved concept / top candidate confidence) from normal, visible UI text alone -\n+  // no DevTools/console/window.__quantityBindingDiagnostics/Node/source inspection. This renders\n+  // one compact block per property resolution (both sides), additive next to the existing\n+  // aggregate summary line - not a new dashboard/panel. concept_id/status/property_context_source/\n+  // property_context_reason/side are all bounded enum values from this module's own fixed\n+  // vocabularies (never raw customer cell text), so no HTML-escaping is required for them.\n+  function renderQuantityPropertyDetail(state) {\n+    const el = $('quantityPropertyDetail');\n+    if (!el) return;\n+    if (!state || !state.ready || !Array.isArray(state.resolutions) || !state.resolutions.length) {\n+      el.innerHTML = '';\n+      return;\n+    }\n+    const fmtConfidence = c => (typeof c === 'number' && Number.isFinite(c)) ? c.toFixed(2) : '—';\n+    const blocks = state.resolutions.map(r => {\n+      const sideLabel = PROPERTY_SIDE_LABELS_JA[r.side] || r.side;\n+      const sourceLabel = PROPERTY_SOURCE_LABELS_JA[r.property_context_source] || r.property_context_source;\n+      const reasonLabel = PROPERTY_CONTEXT_REASON_LABELS_JA[r.property_context_reason] || r.property_context_reason;\n+      const statusLabel = PROPERTY_STATUS_LABELS_JA[r.status] || r.status;\n+      const top = Array.isArray(r.candidates) && r.candidates.length ? r.candidates[0] : null;\n+      const conceptLine = r.status === 'resolved'\n+        ? `判定concept: ${r.concept_id}`\n+        : `判定concept: なし` + (top ? `（参考・最大候補concept: ${top.concept_id}）` : '');\n+      return [\n+        `${sideLabel}`,\n+        `Property文脈: ${sourceLabel} [${r.property_context_source}]`,\n+        `理由: ${reasonLabel} [${r.property_context_reason}]`,\n+        `状態: ${statusLabel}`,\n+        conceptLine,\n+        `最大候補信頼度: ${fmtConfidence(top?.confidence)}`,\n+      ].join('<br>');\n+    });\n+    el.innerHTML = blocks.map(b => `<div style=\"margin-top:6px;\">${b}</div>`).join('');\n+  }\n+\n   function renderQuantityBindingStatus() {\n     const el = $('quantityBindingStatus');\n     if (!el) return;\n     if (!quantityBindingState) {\n       el.textContent = '数量注釈sidecarは未選択です。既存のトレース照合だけを実行します。';\n+      renderQuantityPropertyDetail(null);\n       return;\n     }\n     const s = quantityBindingSummary();";

// HUNK_40 (RE-TOUCHED, Checkpoint 2-D): the Checkpoint 2-C Property-context aggregate summary line
// (unchanged) now also covers, in the same contiguous region: sharing the single
// currentQuantityPropertyState() computation between the aggregate summary and the new per-record
// detail (no double computation), and the trailing renderQuantityPropertyDetail(propState) call.
const HUNK_40 = "     } else {\n       text += ` Phase B-1では比較候補・数値比較・充足判定を生成しません。`;\n     }\n+    // L3-2 Checkpoint 2-C: Property文脈(canonical/legacy)の集約サマリを既存のこの1行へ追記する\n+    // (checkpoint task §16: 新しいダッシュボード・大きなパネルは作らない。既存の最小の詳細面へ)。\n+    // L3-2 Checkpoint 2-D: 集約サマリとper-record detailの両方が同じcurrentQuantityPropertyState()\n+    // の1回の計算結果を共有する(二重計算しない)。\n+    const propState = currentQuantityPropertyState();\n+    const ps = quantityPropertyContextSummary(propState);\n+    if (ps.phase === 'property_stage') {\n+      text += ` Property文脈: Canonical propertyを使用 ${ps.canonicalCount}件`;\n+      if (ps.legacyCount) {\n+        const reasonDetail = Object.entries(ps.reasonCounts)\n+          .filter(([code]) => code !== 'canonical_unique_property')\n+          .map(([code, count]) => `${PROPERTY_CONTEXT_REASON_LABELS_JA[code] || code} ${count}件`)\n+          .join(' / ');\n+        text += ` / 従来の周辺文脈を使用 ${ps.legacyCount}件` + (reasonDetail ? `（理由: ${reasonDetail}）` : '') + '。';\n+      } else {\n+        text += '。';\n+      }\n+    }\n     el.textContent = text;\n+    renderQuantityPropertyDetail(propState);\n   }\n \n   async function loadQuantityBindings() {";
...
// HUNK_42 (new, Checkpoint 2-D): the new #quantityPropertyDetail <div>, inserted immediately after
// the existing #quantityBindingStatus element.
const HUNK_42 = "         <button id=\"loadBtn\" class=\"primary-action\" disabled>読み込み・照合</button>\n       </div>\n       <div id=\"quantityBindingStatus\" class=\"field-hint\" role=\"status\">数量注釈sidecarは未選択です。</div>\n+      <!-- L3-2 Checkpoint 2-D: Human-visible per-record Property detail (side / property_context_source\n+           / property_context_reason / status / concept / top candidate confidence), so a Human reviewer\n+           can evaluate QH-01..QH-06 from normal UI text alone - no DevTools/console/\n+           window.__quantityBindingDiagnostics/Node/source inspection required. Additive only, next to\n+           the existing aggregate summary line above; not a new panel/dashboard. -->\n+      <div id=\"quantityPropertyDetail\" class=\"field-hint\" role=\"status\"></div>\n     </section>\n \n     <section class=\"workflow-section secondary\" aria-label=\"出力・保存\">";

// HUNK_43 (new, Checkpoint 2-D): renderQuantityBindingStatus()'s "unmergeable diagnostics"
// early-return branch also clears the new per-record detail.
const HUNK_43 = "       s.diagnostics.forEach(d => { counts[d.code] = (counts[d.code] || 0) + 1; });\n       const detail = Object.entries(counts).map(([code, count]) => `${code}:${count}`).join(' / ');\n       el.textContent = `数量注釈を結合できません（${detail || '診断なし'}）。不整合データから比較候補・充足判定は生成しません。`;\n+      renderQuantityPropertyDetail(null);\n       return;\n     }\n     const ds = quantityDimensionSummary();";

// HUNK_44 (new, Checkpoint 2-D): invalidateQuantityBindingSelection() also clears the new
// per-record detail, so a stale per-record detail is never left visible after the input selection
// changes.
const HUNK_44 = "     quantityBindingState = null;\n     const el = $('quantityBindingStatus');\n     if (el) el.textContent = '入力選択が変更されました。「読み込み・照合」で数量注釈を再検証してください。';\n+    renderQuantityPropertyDetail(null); // L3-2 Checkpoint 2-D: stale per-record detailを残さない\n   };\n   plmFile.addEventListener('change', invalidateQuantityBindingSelection);\n   sysFile.addEventListener('change', invalidateQuantityBindingSelection);";

const AUTHORIZED_MATCHING_TOOL_DIFF_HUNKS = [HUNK_1, HUNK_2, HUNK_3, HUNK_4, HUNK_5, HUNK_6, HUNK_7, HUNK_8, HUNK_9, HUNK_10, HUNK_12, HUNK_14, HUNK_15, HUNK_16, HUNK_17, HUNK_18, HUNK_19, HUNK_20, HUNK_21, HUNK_22, HUNK_23, HUNK_24, HUNK_25, HUNK_26, HUNK_27, HUNK_28, HUNK_29, HUNK_30, HUNK_31, HUNK_32, HUNK_33, HUNK_34, HUNK_35, HUNK_36, HUNK_37, HUNK_39, HUNK_40, HUNK_41, HUNK_42, HUNK_43, HUNK_44];
